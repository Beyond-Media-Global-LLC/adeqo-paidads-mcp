...
INLINE_CREDENTIALS_ENV = "GOOGLE_ADS_CREDENTIALS_YAML"
INLINE_CREDENTIALS_BASE64_ENV = "GOOGLE_ADS_CREDENTIALS_YAML_BASE64"


def has_ads_credentials() -> bool:
  """Returns whether Google Ads credentials are configured."""
  inline_credentials = os.environ.get(INLINE_CREDENTIALS_ENV)
  inline_credentials_base64 = os.environ.get(INLINE_CREDENTIALS_BASE64_ENV)
  if inline_credentials or inline_credentials_base64:
    return True
...
def get_credentials_source() -> str | None:
  """Describes the configured Google Ads credentials source, if any."""
  if os.environ.get(INLINE_CREDENTIALS_ENV):
    return INLINE_CREDENTIALS_ENV
  if os.environ.get(INLINE_CREDENTIALS_BASE64_ENV):
    return INLINE_CREDENTIALS_BASE64_ENV
  if has_ads_credentials():
    return "GOOGLE_ADS_CREDENTIALS"
  return None


def _load_ads_config() -> tuple[dict[str, Any], str]:
  """Loads the Google Ads YAML configuration from env or disk."""
  inline_credentials = os.environ.get(INLINE_CREDENTIALS_ENV)
  if inline_credentials:
    return yaml.safe_load(inline_credentials), INLINE_CREDENTIALS_ENV

  inline_credentials_base64 = os.environ.get(INLINE_CREDENTIALS_BASE64_ENV)
  if inline_credentials_base64:
    import base64

    decoded = base64.b64decode(inline_credentials_base64).decode("utf-8")
    return yaml.safe_load(decoded), INLINE_CREDENTIALS_BASE64_ENV
  ...
  if not os.path.isfile(credentials_path):
    raise FileNotFoundError(
        ...
        "Set GOOGLE_ADS_CREDENTIALS to a file path or provide "
        "GOOGLE_ADS_CREDENTIALS_YAML."
    )

  with open(credentials_path, "r", encoding="utf-8") as f:
    return yaml.safe_load(f.read()), credentials_path
...
def get_ads_client() -> GoogleAdsClient:
  ...
  ads_config, config_source = _load_ads_config()
  ...
  if not _ADS_CLIENT:
    if config_source in {INLINE_CREDENTIALS_ENV, INLINE_CREDENTIALS_BASE64_ENV}:
      _ADS_CLIENT = GoogleAdsClient.load_from_dict(ads_config)
    else:
      _ADS_CLIENT = GoogleAdsClient.load_from_storage(config_source)
