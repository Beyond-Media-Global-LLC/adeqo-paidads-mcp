...
def test_list_accessible_accounts(mock_google_ads_client):
  ...
  with mock.patch("ads_mcp.tools.api.os.path.isfile", return_value=True):
...
def test_has_ads_credentials_uses_inline_yaml():
  """Inline YAML should count as configured credentials."""
  with mock.patch.dict(
      "os.environ",
      {api.INLINE_CREDENTIALS_ENV: "developer_token: test-token"},
      clear=True,
  ):
    assert api.has_ads_credentials() is True
    assert api.get_credentials_source() == api.INLINE_CREDENTIALS_ENV


@mock.patch("ads_mcp.tools.api.GoogleAdsClient")
def test_get_ads_client_uses_inline_yaml(mock_google_ads_client):
  """Inline YAML should be passed to GoogleAdsClient.load_from_dict."""
  with mock.patch.dict(
      "os.environ",
      {
          api.INLINE_CREDENTIALS_ENV: (
...
@mock.patch("ads_mcp.tools.api.GoogleAdsClient")
def test_get_ads_client_uses_base64_inline_yaml(mock_google_ads_client):
  """Base64-encoded inline YAML should be decoded before loading."""
  encoded = base64.b64encode(b"developer_token: dev-token").decode("utf-8")
  with mock.patch.dict(
      "os.environ",
      {api.INLINE_CREDENTIALS_BASE64_ENV: encoded},
      clear=True,
  ):
    api.get_ads_client()
